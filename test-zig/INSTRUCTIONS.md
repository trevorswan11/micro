...
After that, compiling _should_ be as easy as running `xzig build` (Assuming `xzig` is the custom zig binary). Also keep in mind that this is not the latest version of Zig, specifically, the compiler fork we are using here is based on Zig 0.14.0, which was released mid-2025. This does not matter much for the purposes of this repository and associated work, though. Since we are using an older version of zig, you must also match zls to be 0.14.0. I do this in a project specific `.zed` file, but it will be different per editor.
